...
export default function PlacePage() {
  ...
  const descriptions = Array.isArray(place.description) ? place.description : []
  ...
  return (
    <main className="bg-gradient-to-b from-[#fefcf8] via-white to-[#f0fdf4] text-gray-700">
      <section
        ...
        style={{ backgroundImage: `url('${place.heroImage || place.image || images[0] || ''}')` }}
      >
        ...
        >
          ...
          {place.subtitle && <p className="text-lg md:text-xl text-gray-200">{place.subtitle}</p>}
        </motion.div>
      </section>
              ...
              {images.map((img, i) => (
                <CarouselItem key={img}>
                  <div className="relative w-full h-[60vh]">
                    <Image src={img} alt={`${place.name} gallery ${i + 1}`} fill className="object-cover" />
                  </div>
                </CarouselItem>
              ))}
            </CarouselContent>
      ...
      <section className="px-6 sm:px-10 pb-20 max-w-6xl mx-auto">
        ...
        {stays.length > 0 ? (
          ...
          <p className="text-sm text-gray-500 italic">No stays listed here yet, but we're on it.</p>
        )}
      </section>

      {spots.length > 0 && (
        <section className="px-6 sm:px-10 pb-20 max-w-5xl mx-auto">
          <h2 className="text-2xl font-serif text-emerald-900 mb-6">Scenic Places to Wander</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
            {spots.map((spot, idx) => {
              const spotImages = Array.isArray(spot.images) ? spot.images : []

              return (
                <Dialog key={`${spot.name}-${idx}`}>
